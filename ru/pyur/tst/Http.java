...
public class Http {
    ...
    private final int HTTP_STATE_CLIENT = 2;

    private final int HTTP_STATE_WS_SERVER = 3;  // websocket
    private final int HTTP_STATE_WS_CLIENT = 4;
    private final int HTTP_STATE_WS_CLOSED = 5;

    private final int HTTP_STATE_CAST_SERVER = 6;  // media broadcast
    private final int HTTP_STATE_CAST_CLIENT = 7;
...
*/
    ...
    public int processStream() {
        ...
        // ---------------- if header not received ---------------- //

        if (!isHeaderReceived) {
            result = processStreamHeader();
            ...
            if (state == HTTP_STATE_SERVER) {
                ...
                try {
                    http_request = new HttpRequest(raw_header);
                } catch (Exception e) {
                    e.printStackTrace();
                    return -1;
                }

                //iResult = Http_CB_RequestReceived(http);  // custom request processor

                // else: default request processor
                result = Http_ProcessRequest();
                if (result < 0)  return -1;
                if (result > 0) {
            ...
            else if (state == HTTP_STATE_CLIENT) {
                try {
                    http_response = new HttpResponse(raw_header);
                } catch (Exception e) {
                    e.printStackTrace();
                    return -1;
                }

                result = Http_ProcessResponse();
                if (result < 0)  return -1;
...
//todo                    result = HttpCastResponse(raw_header);
        ...
        if (isHeaderReceived) {
            ...
            //int result = processStreamPayload();
            //if (result < 0)  return -1;

            // ---- HTTP SERVER - receive payload, dispatch and answer ---- //
            if (state == HTTP_STATE_SERVER) {
                System.out.println("12");
                result = Http_ReceivePayload();  // receive POST data, etc...
                if (result < 0) return -1;
                if (result == 0) return 0;

                //if (http->cbDispatch)  result = http->cbDispatch(http->cbInstance, http);
                //else {
                   result = 0;
                //}
                ...
                if (result > 0) {
                    // dispatch success
                    return 1;  // all request payload received
                }

                System.out.println("13");
                Http_SendStandardResponse();  // standard dispatch
            ...
            // ---- HTTP CLIENT - receive payload, and quit ---- //
                ...
                result = Http_ReceivePayload();

                if (result < 0)  return -1;
                if (result == 0)  return 0;

                // -- pass received payload -- //
//todo                if (http->cbPayload)  http->cbPayload(http->cbInstance, http);
                return 1;  // all payload received
            }
    ...
    public int processStreamHeader() {
        ...
        raw_header = Arrays.copyOfRange(stm, 0, header_size);
        ...
        byte[] trail = Arrays.copyOfRange(stm, offset, offset + stm.length );  // 4 is for "\r\n\r\n"
    ...
    // ------------------------ Default Request analyzer ------------------------ //

    private int Http_ProcessRequest() {
        System.out.println("Http_ProcessRequest()");

        //http->request = HttpRequest_Parse(http->lsLines);
...
*/
    ...
    private int Http_ReceivePayload() {
...
//            val++;
    ...
    private void Http_SendStandardResponse() {
        HttpResponse rs = new HttpResponse();
        //r HttpResponse_ConnectionClose(rs);
        rs.setConnectionClose();
        System.out.println("14");

//todo        HttpResponse_Server(rs, Http_CB_GetServerString(http));

        //r rs->szPayload = String_New("hello.");
        rs.appendPayload("Hello sample server.");

        //r Str szResponse = HttpResponse_Stringify(rs);
        //r HttpResponse_Destroy(rs);

        //r Http_SendStr(http, szResponse);
        Http_Send(rs.stringify());

        //r String_Destroy(szResponse);
    }
    ...
    public int Http_Send(byte[] bytes) {
        ...
        //r if (http->cbSend)  return http->cbSend(http->pTransport, data, size);

...
//x    public static final String CONFIG_MODULE_ICON_UPD = "module_icon_upd";
    ...
    public void makeHtml() throws Exception {
        ...
        heading("Генерация спрайтов модулей");
...
//        } catch (Exception e) { e.printStackTrace(); }
        ...
        for (ModuleDesc md : modules) {
            File file = new File("./" + getHostDir() + "/resources/module_icon/" + md.name + ".png");
...
//                return;
        ...
        for (ModuleDesc md : modules) {
            File file = new File("./" + getHostDir() + "/resources/module_icon_2/" + md.name + ".png");
            if (!file.exists())  continue;
...
//        try {
            File output_file = new File(getHostDir() + "/sprite_modules.png");
...
//        try {
            output_file = new File(getHostDir() + "/sprite_modules_2.png");
...
//x        configSet(CONFIG_MODULE_ICON_UPD, timestamp);

        String key = "sprite_modules";
